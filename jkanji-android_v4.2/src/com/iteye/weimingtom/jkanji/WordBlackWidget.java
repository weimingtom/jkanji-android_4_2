...
public class WordBlackWidget extends AppWidgetProvider {
	...
	public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(ACTION_UPDATE)) {
            ...
            int[] appWidgetIds = appWidgetManager.getAppWidgetIds(new ComponentName(context, WordBlackWidget.class));
    ...
    private static RemoteViews buildUpdate(Context context) {
		...
		if (total > 0) {
			...
			if (word_kanji == null) {
				word_kanji = word_reading;
			}
		}
        RemoteViews updateViews = null;
        updateViews = new RemoteViews(context.getPackageName(), R.layout.widget_black_word);
        updateViews.setTextViewText(R.id.word_reading, word_reading);
        updateViews.setTextViewText(R.id.word_name, word_kanji);
        ...
        updateViews.setOnClickPendingIntent(R.id.widget_black, pendingIntent);
        return updateViews;
    }

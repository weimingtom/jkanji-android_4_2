...
public class GPUImageView extends FrameLayout {
     ...
     */
    public void saveToPictures(final String fileName,
                               final OnPictureSavedListener listener) {
        new SaveTask(fileName, listener).execute();
    }
    ...
    public void saveToPictures(final String fileName,
                               ...
                               final OnPictureSavedListener listener) {
        new SaveTask(fileName, width, height, listener).execute();
    }
    ...
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        public SaveTask(final String fileName,
                        final OnPictureSavedListener listener) {
            this(fileName, 0, 0, listener);
        }

        public SaveTask(final String fileName, int width, int height,
                        final OnPictureSavedListener listener) {
            mFileName = fileName;
            mWidth = width;
            mHeight = height;
        ...
        private void saveImage(final String fileName, final Bitmap image) {
            //final File file = new File(folderName + File.separator + fileName);
            FileOutputStream os = null;
        	try {
//                file.getParentFile().mkdirs();
        		/**
        		 * @see http://androidforums.com/application-development/25988-send-image-action_send.html
        		 */
        		os = getContext().openFileOutput(fileName, Context.MODE_WORLD_READABLE);
                boolean result = image.compress(Bitmap.CompressFormat.JPEG, 100, os);
                if (result) {

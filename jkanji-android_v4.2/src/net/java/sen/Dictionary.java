...
public class Dictionary {
  ...
  public CToken[] commonPrefixSearch(char str[], int pos) throws IOException {
    int size = 0;
    if (D) {
      Log.d(TAG, "input=" + new String(str, pos, str.length - pos));
      Log.d(TAG, "pos=" + pos);
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = 0; j < k; j++) {
        ...
        if (D) {
        	Log.d(TAG, "----found----");
        	Log.d(TAG, "posInfo=" + this.getPosInfo(result[size].posID));
        	Log.d(TAG, "rcAttr2=" + result[size].rcAttr2);
        	Log.d(TAG, "rcAttr1=" + result[size].rcAttr1);
        	Log.d(TAG, "lcAttr=" + result[size].lcAttr);
        	Log.d(TAG, "length=" + result[size].length);
        	Log.d(TAG, "cost=" + result[size].cost);
        }
    ...
    if (n != -1) {
      ...
      for (int i = 0; i < k; i++) {
        ...
        if (D) {
        	...
        	Log.d(TAG, "lcAttr=" + result[size].lcAttr);
        	Log.d(TAG, "length=" + result[size].length);
        	Log.d(TAG, "str=" + new String(str, pos, result[size].length));
        }
        size++;
      }
    }
  ...
  public Dictionary(String tokenFile, String doubleArrayFile,
    ...
    if (D) {
    	Log.i(TAG, "token file = " + tokenFile);
    }
    start = System.currentTimeMillis();
    tfd = FileAccessorFactory.getInstance(tokenFile);
    if (D) {
	    Log.i(TAG, "time to load posInfo file = "
	        + (System.currentTimeMillis() - start) + "[ms]");
    }
    // load double array trie dictionary
    if (D) {
    	Log.i(TAG, "double array trie dictionary = " + doubleArrayFile);
    }
    da.load(doubleArrayFile);

    // open pos infomation file.
    if (D) {
    	Log.i(TAG, "pos info file = " + posInfoFile);
    }
    start = System.currentTimeMillis();
    ffd = FileAccessorFactory.getInstance(posInfoFile);
    if (D) {
    	Log.i(TAG, "time to load pos info file = "
    			+ (System.currentTimeMillis() - start) + "[ms]");
    }
 }
  ...
  public CToken getBOSToken() throws IOException {
    ...
    if (D) {
    	Log.d(TAG, "getBOSToken()");
    	Log.d(TAG, "rcAttr2 = " + t.rcAttr2);
    	Log.d(TAG, "rcAttr1 = " + t.rcAttr1);
    	Log.d(TAG, "lcAttr = " + t.lcAttr);
    	Log.d(TAG, "posid = " + t.posid);
    	Log.d(TAG, "length = " + t.length);
    	Log.d(TAG, "cost = " + t.cost);
    	Log.d(TAG, "posID = " + t.posID);
    }
    return t;
  }

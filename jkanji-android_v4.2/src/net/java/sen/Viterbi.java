...
public class Viterbi {
	...
	private final static String TAG = "Viterbi";
  ...
  public synchronized Node analyze(char sentence[]) throws IOException {
    ...
    if (D) {
    	Log.d(TAG, "sentence = \"" + new String(sentence) + "\"");
    }
    // initialize Viterbi
    init();
    ...
    for (int pos = 0; pos < len; pos++) {
      if (endNodeList[pos] != null) {
        ...
        if (rNode != null) {
          ...
          if (D) {
        	  Log.d(TAG, "rNode.token.rcAttr2=" + rNode.token.rcAttr2);
        	  Log.d(TAG, "rNode.token.rcAttr1=" + rNode.token.rcAttr1);
        	  Log.d(TAG, "rNode.token.lcAttr=" + rNode.token.lcAttr);
          }
        }
      }
    }
  ...
  final private void calcConnectCost(int pos, Node rNode) throws IOException {
    ...
    for (; rNode != null; rNode = rNode.rnext) {
      ...
      for (Node lNode = endNodeList[pos]; lNode != null; lNode = lNode.lnext) {
        if (D) {
        	Log.d(TAG, "candidate morpheme = " + lNode.toString());
        	Log.d(TAG, "token id = " + lNode.token.posID);
        	Log.d(TAG, "lnode.prev(" + lNode.prev.token.rcAttr2 + ") = "
              + tokenizer.dic.getPosInfo(lNode.prev.token.posID));
        	Log.d(TAG, "lnode(" + lNode.token.rcAttr1 + ") = "
              + tokenizer.dic.getPosInfo(lNode.token.posID));
        	Log.d(TAG, "rnode(" + rNode.token.lcAttr + ") = "
      ...
      if (rNode.token.rcAttr2 != 0) {
        ...
        for (; rNode2 != null; rNode2 = rNode2.rnext) {
          if (D) {
        	  Log.d(TAG, "rnode.prev(" + rNode.prev.token.rcAttr2 + ") = "
                + tokenizer.dic.getPosInfo(rNode.prev.token.posID));
        	  Log.d(TAG, "rnode(" + rNode.token.rcAttr1 + ") = "
                + tokenizer.dic.getPosInfo(rNode.token.posID));
        	  Log.d(TAG, "rnode2(" + rNode2.token.lcAttr + ") = "

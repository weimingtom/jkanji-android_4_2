...
public class DoubleArrayTrie {
  ...
  private final static String TAG = "DoubleArrayTrie";
  ...
  public void load(String fileName) throws IOException {
    if (D) {
    	Log.i(TAG, "loading double array trie dict = " + fileName);
    }
    ...
    if (D) {
    	Log.i(TAG, "loaded time = " + (((double)(System.currentTimeMillis()- start))/1000) + "[ms]");
    }
  }
  ...
  int fetch (Node parent, Vector siblings)
  {
    int prev = 0;
    if(D) {
      Log.d(TAG, "parent.left=" + parent.left);
      Log.d(TAG, "parent.right=" + parent.right);
      Log.d(TAG, "parent.depth=" + parent.depth);
    }
    ...
    for (int i = parent.left; i < parent.right; i++) {
      ...
      if (((len != null) ? len[i] : str[i].length) != parent.depth) {
        if(D)
        	Log.d(TAG, "tmp["+parent.depth+"]="+tmp[parent.depth]);
                    ...
                    int size)
  {
    ...
    if (_str.length != _val.length) {
    	if (D) {
    		Log.w(TAG, "index and text should be same size.");
    	}
    	return 0;
    }
                                 ...
                                 int len)
  {
    ...
    for ( int i = pos; i < len; i++) {
      ...
      if ((int) b == array[(((int)p) << 1) + 1] && n < 0) {
        if(D) {
        	Log.d(TAG, "result["+num+"]="+(-n-1));
        }
        if (num < result.length) {
        	result[num] = -n-1;
        } else {
        	if (D) { 
        		Log.w(TAG, "result array size may not enough");
        	}
        }
        num++;
      }
      ...
      if ( (p<<1) > array.length) {
    	  if (D) {
    		  Log.w(TAG, "p range is over.");
    		  Log.w(TAG, "(p<<1,array.length)=("+(p<<1)+","+array.length+")");
    	  }
    	  return num;
      }

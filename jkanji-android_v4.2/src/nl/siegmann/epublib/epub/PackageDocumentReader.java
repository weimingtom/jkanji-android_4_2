...
public class PackageDocumentReader extends PackageDocumentBase {
	...
	private final static String TAG = "PackageDocumentReader";
...
//	}
	...
	private static Resources readManifest(Document packageDocument, String packageHref,
		...
		if(manifestElement == null) {
			if (D) {
				Log.e(TAG, "Package document does not contain element " + OPFTags.manifest);
			}
			return result;
		}
	...
	private static void readGuide(Document packageDocument,
		...
		for (int i = 0; i < guideReferences.getLength(); i++) {
			...
			if (resource == null) {
				if (D) {
					Log.e(TAG, "Guide is referencing resource with href " + resourceHref + " which could not be found");
				}
				continue;
			}
			...
			if (StringUtil.isBlank(type)) {
				if (D) {
					Log.e(TAG, "Guide is referencing resource with href " + resourceHref + " which is missing the 'type' attribute");
				}
				continue;
			}
	...
	private static Spine readSpine(Document packageDocument, EpubReader epubReader, Resources resources, Map<String, String> idMapping) {
		...
		if (spineElement == null) {
			if (D) {
				Log.e(TAG, "Element " + OPFTags.spine + " not found in package document, generating one automatically");
			}
	...
	private static Resource findTableOfContentsResource(Element spineElement, Resources resources) {
		...
		if (tocResource == null) {
			if (D) {
				Log.e(TAG, "Could not find table of contents resource. Tried resource with id '" + tocResourceId + "', " + Constants.DEFAULT_TOC_ID + ", " + Constants.DEFAULT_TOC_ID.toUpperCase() + " and any NCX resource.");
			}
		}
		return tocResource;
	}
